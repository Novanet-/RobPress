...
class Blog extends Controller
{
    ...
    public function comment($f3)
    {
        ...
        if ($this->request->is('post')) {
            ...
            $comment->copyfrom('POST', function ($arr) { //Only allow parameters from this given set (PARAM MANIPULATION)
...
//            $ids = $this->db->connection->exec("SELECT id FROM `posts` WHERE `title` LIKE \"%$search%\" OR `content` LIKE '%$search%'");
            ...
            if (empty($ids)) {
                StatusMessage::add('No search results found for ' . h($search)); //XSS protection
